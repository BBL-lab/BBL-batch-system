...
%%%
%%% Inputs:
%%% y = a column vectorof data mated to the design specified in F
%%%
...
elseif nargin == 3
    try
        [c cc] = getC(custom,F.XX,F.CovarCols);
        c0 = eye(size(x,2))-(c*pinv(c));
        x0 = x*c0;
        r = eye(size(x0,1))-(x0*pinv(x0));
    catch
        if ~isfield(custom,'c') || isempty(custom.c)
            [r c c2] = MakeContrastMatrix('a',custom.DesignGroup, custom.levs,xx,x);
            %[r c c2] = MakeContrastMatrix('a',F.XX(:,custom.DesignGroup), custom.levs,xx,x);
        else
            c = custom.c;
            c0 = eye(size(x,2))-(c*pinv(c));
            x0 = x*c0;
    ...
    F.effCon{custom.ET,end+1} = c;
    F.EffectSS{custom.ET,end+1} = SS;
    F.EffectDF(custom.ET,end+1) = df1;
    
    df2 = F.ErrorDF(custom.ET);
    SS1 = SS;
    SS2 = F.ErrorSS{custom.ET};
    MS1 = SS1/df1;
    MS2 = SS2/df2;
    F1 = MS1./MS2;
    p = 1-cdf('f',F1,df1,df2);

function RunSPM8ana(F,scans,opt,pars)
...
if ~exist('pars');
    pars.dept = CCN.dept;  %% Assume Dependence?       0=no or 1=yes
    pars.var = CCN.var;   %% Assume Unequal Variance?   0=no or 1=yes
%    pars.tm.tm_none = CCN.tm.tm_none;    %% No Threshold Masking
     pars.tm.tma.athresh= CCN.tm.tma.athresh;   %% Absolute theshold masking
%     pars.tm.tmr.rthresh= 0.80;  %% Relative theshold masking
    pars.im = CCN.im;    %% Implicit Masking   1 = yes, 0 = no
    pars.em = CCN.em; %% Explicit Masks     list mask files e.g. {'/autofs/space/plato_002/users/APS_MATLAB/spm8/templates/epi.nii,1'};
    
   % pars.g_omit = CCN.g_omit;    %% Global Calculation:   1 is the default setting
    pars.g_user.global_uval = CCN.g_user.global_uval;
    pars.gmsca.gmsca_no = CCN.gmsca.gmsca_no;  %% No Grand Mean Scaling.
%     pars.gmsca.gmsca_yes.gmscv = 50;
    pars.glonorm = CCN.glonorm;   %% Global Normalization.  1 = No. 2 = Proportional 3 = ANCOVA
end
...
if ~isempty(in.Within);
    ...
    if ~isempty(in.Between);
        if ~isempty(in.Covar_1); % added by Jerome Prado to allow for regression
            for ii = 1:length(in.Covar)
            cc = cc+1;
            job.des.mreg.scans = F.Scans';
            job.des.mreg.mcov(cc).c = in.Covar{cc};
            job.des.mreg.mcov(cc).cname = in.CovarLabs{cc};
            job.des.mreg.mcov(cc).iCC = in.Centering;
            job.des.mreg.incint = in.Intercept;
            end
        else
...
if isempty(in.Covar); % added by Jerome Prado to allow for regression
...
job.globalc.g_user.global_uval = pars.g_user.global_uval;
...
spm_run_factorial_design_old(job);
spm_run_fmri_est_old(job2);
if isfield(in,'con')
    spm_run_con(job3);
end

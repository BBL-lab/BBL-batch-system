...
if nargin==1
    ...
    if ok == 0;
        ...
        if isfield(I,'Scans');
            ...
            DD = double(DD);
            %VY = spm_vol(char(I.Scans));
            %D = double(spm_read_vols(VY));
...
ss = I.v.dim;
for ii = 1:numel(I.F.err);
    ResMS{ii} = zeros(ss)*NaN;
end
NN = zeros(ss)*NaN;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ll = size(X,1):-1:I.minN
    ...
    %persisText(['Processing Set #' num2str((size(X,1)+1)-ll) ' of ' num2str(size(X,1))]);
    ...
    for ii = 1:size(ch,1)
        ...
        %if ii == 1;
        %    disp(['Processing Set #' num2str((size(X,1)+1)-ll) ' of ' num2str(size(X,1))]);
        %end
        ...
        if I.RemoveOutliers == 1;
            ...
            if isfield(I,'Thresh') && ~isempty(I.Thresh)
                %thresh = I.Thresh+(4./size(xx,1));
                %warning('Really Dude?  Really?');
        ...
        if numel(I.F.Vi)>1
            ...
            try
                ...
                %facInd = setdiff(contains('^F_.*',I.F.name),contains('.*S.*',I.F.name));
                %xxW = xx(:,facInd);
                [V h] = spm_reml_aps(Cy,xxW,Vi);
            catch
                disp('No go');
                continue;
            end

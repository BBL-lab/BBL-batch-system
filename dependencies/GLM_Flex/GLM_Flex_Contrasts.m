...
%    persisText
    for jj = 1:length(oo.Xind)
        ...
        try
            ...
            if size(c,2)>1
                Vol2(oo.vec{jj}) = F;
                %fname = ['apsF_' lab '.nii'];
                fname = [lab '_F_' Cons(ii).name '.nii'];
            else
                Vol2(oo.vec{jj}) = sqrt(F).*sign(c'*beta);
                Vol4(oo.vec{jj}) = c'*beta;
                %fname = ['apsT_' lab '.nii'];
                fname = [lab '_T_' Cons(ii).name '.nii'];
            end
    ...
    writeIMG(h,Vol2,fname);
    %spm_write_vol(h,Vol2);
    
    fname = ['ess_' lab '.nii'];
    h.descrip = [];
    writeIMG(h,Vol1,fname);
    
    for jj = 1:length(MeanVol)
        for kk = 1:length(MeanVol{jj});
            fname = ['MeanVol_C' num2str(jj) '_G' num2str(kk) '.nii'];
            writeIMG(h,MeanVol{jj}{kk},fname);
        end
    end
    
    if size(c,2) == 1;
        fname = ['con_' lab '.nii'];
        writeIMG(h,Vol4,fname);
    end
end

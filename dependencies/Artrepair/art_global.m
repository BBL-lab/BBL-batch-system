...
%  Alternatively, deviations over 2*std are outliers, if std is not very small.
      z_thresh = CCN.z_thresh;  % Currently not used for default.
...
% set this threshold higher.
      mv_thresh = CCN.mv_thresh;  % try 0.3 for subjects with intervals with low noise
...
if nargin > 0
    ...
    for i = 1:num_sess
        ...
        M{i} = load(mvmt_file(~isspace(mvmt_file)));
